...
- **Tests:** `node --test` (NOT vitest). Root `pnpm test` runs: (1) `bash agent/scripts/test-phase-b-storage-paths.sh` grep guard against legacy flat-layout storage paths, (2) strict `tsc --noEmit` on agent + frontend, (3) **the 407 unit tests** — `pnpm -C agent exec node --import=tsx --test "test/*.test.ts"` (350 agent) + the frontend `hidden-runs` suite via agent's tsx (57). Unit-test wiring added S96 (2026-06-05); pre-S96 the script ran only the grep guard + dual tsc (tests existed but never gated). NOTE: Node 22 `node --test` only auto-discovers `*.test.js`; passing an explicit glob arg (`"test/*.test.ts"`) is what makes it find the `.ts` tests.
