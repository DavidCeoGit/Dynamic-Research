...
- **Tests:** `node --test` (NOT vitest). Root `pnpm test` runs: (1) `bash agent/scripts/test-phase-b-storage-paths.sh` grep guard against legacy flat-layout storage paths, (2) strict `tsc --noEmit` on agent + frontend, (3) **the 446 unit tests** — `pnpm -C agent exec node --import=tsx --test "test/*.test.ts"` (381 agent) + the frontend suites via agent's tsx (65: hidden-runs + attachments + storage-attachments). Unit-test wiring added S96 (2026-06-05); `storage-attachments.test.ts` (8 tests) added S110; `staging-sweep.test.ts` grew to 30 tests at S112 (sweep fan-out budget + circular-ring resume) and to 40 at S113 (delete-phase budget + ring-generation prune + monotonic budget clock). NOTE: Node 22 `node --test` only auto-discovers `*.test.js`; passing an explicit glob arg (`"test/*.test.ts"`) is what makes it find the `.ts` tests.
