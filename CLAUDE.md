...
- **Tests:** `node --test` (NOT vitest). Root `pnpm test` runs: (1) `bash agent/scripts/test-phase-b-storage-paths.sh` grep guard against legacy flat-layout storage paths, (2) strict `tsc --noEmit` on agent + frontend, (3) **the unit tests** (S178 measured: `pnpm test` reports `tests 663` agent + `tests 134` frontend = 797; the `pnpm test` command is the source of truth — exact counts drift with development) — `pnpm -C agent exec node --import=tsx --test "test/*.test.ts"` (663 agent) + the frontend suites via agent's tsx (134: hidden-runs + attachments + storage-attachments + publish-flag + route-protection + agent-auth + context-extraction + run-status + studio-products + the 4 repo-root cross-tier parity guards publish-flag-parity + studio-products-parity + storage-paths-parity + attachments-constants-parity). Unit-test wiring added S96 (2026-06-05); `storage-attachments.test.ts` (8 tests) added S110; `staging-sweep.test.ts` grew through S112/S113 to 47 tests by S145; S164 added 4 agent suites from the S163 audit coverage-gap finding (`untrusted-input` 19 + `usage-tracking` 23 + `api-client` 18 + `notify` 20 = +80 → 581 agent); S178 added two repo-root parity guards — `test/storage-paths-parity.test.ts` (4) + `test/attachments-constants-parity.test.ts` (5, the agent↔frontend attachment-constants ↔ conventions.json mirror: caps/types/regex/reserved-basenames) — both closing audit 2026-06-24 MEDIUMs ("no sync enforcement"). (Prior counts "581 agent / 102 frontend = 683" were the S164 snapshot; "446 / 381 agent / 65 frontend / staging-sweep 40" a frozen S113 snapshot.) NOTE: Node 22 `node --test` only auto-discovers `*.test.js`; passing an explicit glob arg (`"test/*.test.ts"`) is what makes it find the `.ts` tests.
