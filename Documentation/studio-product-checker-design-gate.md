...
| 10 | **Wedged child post-Studio** (the S133 signature — fresh-Claude M-2) | ALL selected products `status_id == 3` AND newest matched artifact `created_at` ≥ 25 min ago AND row still `running` AND child alive (#6 guard passes) | 2 sightings | `CHILD_WEDGED_POST_STUDIO` (FYI) |
